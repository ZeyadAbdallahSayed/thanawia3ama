...
const RecognizePage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!user) {
      user = { id: Date.now(), userName: trimmedName };
      users.push(user);
      localStorage.setItem("admin-users", JSON.stringify(users));
    }
  ...
  return (
    <div
      ...
      style={{ backgroundImage: `url(${backgroundd0})` }}
    >
      <div className="sm:w-full lg:w-1/2 flex flex-col items-center justify-center gap-5 mx-auto -mt-38 bg-transparent rounded-xl  z-[10] p-20 shadow-2xl">
        <h1 className="sm:text-3xl text-4xl text-white font-[fantasy]">Thanawia 3ama Organizer</h1>
        <h2 className="text-3xl text-zinc-800 font-[fantasy]">Welcome Hero...</h2>
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <input
            ...
            className="text-l text-black font-[fantasy] p-3 w-[16rem] rounded-xl mt-6 border-none outline-none"
          />
          {error && <p className="text-red-600 font-[fantasy]">{error}</p>}
          <button
            type="submit"
